...
-- These UPDATE statements add color hex codes to materials
-- so they render with proper colors instead of hash fallbacks
-- ============================================================
...
-- ============================================================
-- PHASE 3: HOSTILE MOBS (Added v61)
-- ============================================================

-- Function to spawn hostile mobs automatically
CREATE OR REPLACE FUNCTION spawn_hostile_mobs_in_world()
RETURNS TABLE(world_id uuid, world_name text, mobs_spawned integer) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_total integer := 0;
BEGIN
  SELECT active_world_id INTO v_world_id
  FROM player_profiles WHERE user_id = auth.uid();
  
  IF v_world_id IS NULL THEN
    SELECT w.id INTO v_world_id
    FROM worlds w ORDER BY w.created_at DESC LIMIT 1;
  END IF;
  
  IF v_world_id IS NULL THEN
    RAISE EXCEPTION 'No world found. Create a world first!';
  END IF;
  
  SELECT name INTO v_world_name FROM worlds WHERE id = v_world_id;
  
  -- Spawn 5 zombies
  INSERT INTO mobs (world_id, type, x, y, z, yaw, hp)
  SELECT v_world_id, 'zombie',
    (100 + (random() * 40 - 20))::real, 50::real,
    (100 + (random() * 40 - 20))::real,
    (random() * 6.28)::real, 20::smallint
  FROM generate_series(1, 5);
  v_total := v_total + 5;
  
  -- Spawn 5 skeletons
  INSERT INTO mobs (world_id, type, x, y, z, yaw, hp)
  SELECT v_world_id, 'skeleton',
    (100 + (random() * 40 - 20))::real, 50::real,
    (100 + (random() * 40 - 20))::real,
    (random() * 6.28)::real, 20::smallint
  FROM generate_series(1, 5);
  ...
  RETURN QUERY SELECT v_world_id, v_world_name, v_total;
END;
$$;
